...
nBigshiftHosp = c("related"   =length(which(ssti$dxScabies==1)),
                  "nonrelated"=length(which(ssti$dxScabies==2)))
...
mCost <- list(
  ...
  related    = mean(ssti$cMeanTotal[ssti$dxScabies==1]),
  nonrelated = mean(ssti$cMeanTotal[ssti$dxScabies==2]))
mCost
...
cTempLow  <- nTotalHosp["related"] * mean(ssti$cMeanTotal_cWardAdmit_low[ssti$dxScabies==1])
cTempHigh <- nTotalHosp["related"] * mean(ssti$cMeanTotal_cWardAdmit_high[ssti$dxScabies==1])
...
cTempLow  <- nTotalHosp["related"] * mean(ssti$cMeanTotal_cICUAdmit_low[ssti$dxScabies==1])
cTempHigh <- nTotalHosp["related"] * mean(ssti$cMeanTotal_cICUAdmit_high[ssti$dxScabies==1])
cOwsaIcu  <- c("Low"  = unname(sum(cTotalPhc) + cTempLow),
               "High" = unname(sum(cTotalPhc) + cTempHigh))

# Sensitivity analysis on cost per laboratory test
cTempLow  <- nTotalHosp["related"] * mean(ssti$cMeanTotal_cDiagnostics_low[ssti$dxScabies==1])
cTempHigh <- nTotalHosp["related"] * mean(ssti$cMeanTotal_cDiagnostics_high[ssti$dxScabies==1])
...
#          "Procedure, surgical"      = txProcSurgery,
#          "Procedure, amputation"    = txProcAmputat,


# ggplot(ssti, aes(dHospStay)) +
#   geom_histogram(fill = "#FEDAB8", color = "black", binwidth = 6) +
#   xlab("Number of hospital bed days") + ylab("Count") +
#   theme_bw() +
#   theme(axis.title        = element_text(size = 15), 
#         axis.text         = element_text(size = 13,  color = "black"),
#         axis.line         = element_line(linewidth = 0.05, color = "#444444"),
#         axis.ticks        = element_line(linewidth = 0.3, color = "black"),
#         axis.ticks.length = unit(0.1, "cm"),
#         panel.grid.major  = element_line(linewidth = 0.2, colour = "gray80")) +
#   scale_x_continuous(breaks=seq(0,  110,  15)) +
#   facet_wrap(~dxScabies)
# ggsave(height=6, width=8, dpi=600, file="output/histogram-hospital-stay.png")
# 
# 
# # Export tables to Microsoft Word
# word_export <- read_docx()
# word_export <- word_export %>% 
#   # body_add_par(value = "Table 1") %>%
#   # body_add_flextable(table1) %>% 
#   # body_add_break() %>% 
#   body_add_par(value = "Table 2") %>% 
#   body_add_flextable(table2) %>%
#   body_add_break() %>%
#   body_add_par(value = "Table 3") %>%
#   body_add_flextable(sTable2) %>%
#   body_add_break() %>%
#   # body_add_par(value = "Table 4") %>%
#   # body_add_flextable(table4) %>%
#   # body_add_break() %>%
#   # body_add_par(value = "Supplementary Table 1") %>%
#   # body_add_flextable(sTable1) %>%
#   # body_add_break() %>%
#   # body_add_par(value = "Supplementary Table 2") %>%
#   # body_add_flextable(sTable2) %>%
#   # body_add_break() %>%
#   # body_add_par(value = "Supplementary Table 3") %>%
#   # body_add_flextable(sTable3) %>%
#   # body_add_break() %>%
#   # body_add_par(value = "Supplementary Table 4") %>%
#   # body_add_flextable(sTable4) %>%
#   # body_add_break() %>%
#   # body_add_par(value = "Supplementary Table 5") %>%
#   # body_add_flextable(sTable5) %>%
#   print('output/tables.docx')
# 
# 
# 
# 
# # For Mean number of hospital days (report)
# mean(ssti$dHospStay)
# mean(ssti$dHospStay[ssti$dxScabies=="Potentially scabies-related"])
# mean(ssti$dHospStay[ssti$dxScabies=="Unlikely scabies-related"])
# 
# 
# mean(phc$txMedsInjectn)
# mean(phc$txMedsInjectn[phc$dxScabies=="Scabies presentation"])
# mean(phc$txMedsInjectn[phc$dxScabies=="Non-scabies presentation"])
# 
# 
# 
# mean(phc$txMedsInjectn[phc$ageD=="0-4"], na.rm = TRUE)
# mean(phc$txMedsInjectn[phc$ageD=="5-9"], na.rm = TRUE)
# mean(phc$txMedsInjectn[phc$ageD=="10-14"], na.rm = TRUE)
# mean(phc$txMedsInjectn[phc$ageD=="15+"], na.rm = TRUE)
